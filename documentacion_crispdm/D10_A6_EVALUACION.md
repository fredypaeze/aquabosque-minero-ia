...
Como se mencionó en el modelado, el modelo es honesto en que la etiqueta es una fórmula compuesta, por lo que el modelo re-aprende parcialmente la regla. Esta honestidad metodológica tiene implicaciones importantes:

1. **No se vende la exactitud como mérito predictivo**: La accuracy es alta por construcción
2. **El valor del modelo es la explicabilidad**: SHAP permite entender qué factores influyen
3. **Se reporta la línea base**: Para contexto de comparación
